...
##################################
	...
	IF ((get-mailbox -identity $AccountToDisable -ErrorAction SilentlyContinue))
	{
	    ...
	    $MailboxExport = $TEMPMAILPATH + $AccountToDisable + "-" + $date + ".pst"
    	...
    	move-item $MailboxExport $HOMEARCHIVEPATH
...
##################################
	...
	# Export Group Memberships and Distribution List Memberships to CSV, then remove from groups
	$grouptarget = $HOMEARCHIVEPATH + "\" + $AccountToDisable + "-Groups-" + $date + ".csv"
